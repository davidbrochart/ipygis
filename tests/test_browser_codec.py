...
class TestBrowserCodec:
    ...
    async def setup_store(self):
        ...
        self.data = {key: (await memory.get(key, default_buffer_prototype())).to_bytes()
        ...
        self.connection = SimpleNamespace(
            decode_lzw=decode, _ready=anyio.Event(),
            _widget=SimpleNamespace(comm=object()),
        )
        self.connection._ready.set()
        self.connection._widget.close = lambda: setattr(self.connection._widget, 'comm', None)
        with patch('ipygis.gis.GIS', return_value=self.connection) as self.factory, \
    ...
    def open_dataset(self, store=None):
        return xr.open_zarr(self.store if store is None else store, chunks=None,
                            consolidated=False, create_default_indexes=False)
    ...
    async def test_sequential_reads_reuse_the_decoder_connection(self):
        ...
        for index in (0, 1):
            ...
            assert result.item() == (7. if index == 0 else 8.)
        self.factory.assert_called_once()
    ...
    async def test_codec_configuration_roundtrips_without_runtime_data(self):
        ...
        with pytest.raises(ValueError, match='configuration'):
            BrowserLzwCodec.from_dict({'name': 'imagecodecs_lzw', 'configuration': {'unsupported': True}})
