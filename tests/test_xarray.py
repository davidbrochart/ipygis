...
class Transport:
    ...
    async def array_request(self, operation, *, path, **kwargs):
        ...
        if operation == 'zarr_open':
            ...
            return dict(kind='array', attrs=attrs, shape=value.shape, chunks=value.shape,
                        dimension_names=None if self.v2 else self.dims[name], dtype='string' if value.dtype.kind == 'O' else str(value.dtype)), []
        self.reads.append(name)
        key = tuple(slice(*s) if isinstance(s, list) else s for s in kwargs['selection'])
        result = np.asarray(value[key]).copy()
        if value.dtype.kind == 'O':
            return dict(shape=result.shape, dtype='string', values=result.ravel().tolist()), []
        return dict(shape=result.shape, strides=[s // result.itemsize for s in result.strides],
                    dtype=str(result.dtype), byteorder='<'), [result.tobytes()]
...
async def test_coordinates_and_lazy_selection(v2):
    ...
    vector = await ds.isel(y=xr.DataArray([2, 0], dims='points'), x=xr.DataArray([3, 1], dims='points')).load_async()
...
async def test_metadata_only_and_lifetime():
    ...
    np.testing.assert_array_equal((await ds.isel(y=1, x=slice(1, 3)).load_async()).elevation, [5, 6])
    ds.close()
...
async def test_cf_scale_mask_and_time():
    backend = Transport()
    backend.attrs['elevation'] = {'scale_factor': 2., 'add_offset': 1., '_FillValue': 0.}
    ...
    assert raw.elevation.attrs['scale_factor'] == 2.
...
async def test_encoded_fill_value_and_auxiliary_coordinate():
    backend = Transport()
    backend.attrs['elevation']['_FillValue'] = base64.standard_b64encode(struct.pack('<d', 0.)).decode()
    backend.values['latitude'] = np.array([50., 49., 48.])
    ...
    assert result.latitude.item() == 50.
    np.testing.assert_array_equal(result.elevation, [np.nan, 1., 2., 3.])
