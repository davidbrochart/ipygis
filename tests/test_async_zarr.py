...
class Backend:
    ...
    async def array_request(self, operation, **args):
        ...
        if operation == 'zarr_open':
            ...
            return {'kind': 'array', 'attrs': {}, 'shape': [4, 6], 'chunks': [2, 3], 'dtype': 'int32'}, []
        selection = tuple(slice(*s) if isinstance(s, list) else s for s in args['selection'])
        data = np.asarray(self.data[selection]).copy()
        return {'shape': data.shape, 'strides': [s // data.itemsize for s in data.strides], 'dtype': 'int32', 'byteorder': '<'}, [memoryview(data.tobytes())]
...
@pytest.mark.parametrize('key', [
    (slice(1, 4, 2), slice(1, 6, 2)), (-1, 2), (Ellipsis, -1),
    (), slice(None), (slice(3, 1), slice(None)), (slice(-100, 100), 0),
])
...
@pytest.mark.parametrize('key,error', [
    ((4, 0), IndexError), ((-5, 0), IndexError),
    ((0, 0, 0), IndexError), ((Ellipsis, Ellipsis), IndexError),
    ((slice(None, None, 0),), ValueError),
    ((slice(None, None, -1),), NotImplementedError),
    (([1, 2],), TypeError), ((None,), TypeError), ((True,), TypeError),
])
...
async def test_strided_big_endian_reply():
    class Strided(Backend):
        async def array_request(self, operation, **args):
            ...
            return {'shape': [4, 6], 'strides': [1, 4], 'dtype': 'int32', 'byteorder': '>'}, [data.tobytes(order='F')]
    array = await zarr.open_array(Strided(), path='a')
    np.testing.assert_array_equal(await array.getitem(Ellipsis), np.arange(24).reshape(4, 6))
...
async def test_array_transport_is_independent_of_byte_backend():
    ...
    store = SimpleNamespace(backend=object(), array_backend=BrowserArrayBackend(request))
