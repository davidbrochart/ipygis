    ...
    async def _read(self, key):
        ...
        for source, offsets in self.sources:
            ...
            for axis, dim in enumerate((self.y, self.x)):
                ...
                offset = offsets[axis]
                pos = np.flatnonzero(
                    (indices >= offset) & (indices < offset + source.sizes[dim])
                )
                if not len(pos):
                    break
...
async def mosaic_async(
    sources, *, x='x', y='y', crs, fill_value=np.nan
) -> xr.DataArray:
    ...
    if dtype.kind in 'iu':
        ...
        if (
            not isinstance(fill_value, (int, np.integer))
            or not limits.min <= fill_value <= limits.max
        ):
            raise ValueError(
                'Integer sources require an explicit representable integer fill_value'
            )
    ...
    for source in sources:
        ...
        for dim, direction in ((y, -1), (x, 1)):
            if dim not in source.coords or source.coords[dim].dims != (dim,):
                raise ValueError(
                    f'{dim!r} requires a one-dimensional pixel-center coordinate'
                )
            ...
            if len(values) < 2 or not np.all(np.isfinite(values)):
                raise ValueError(
                    'Each spatial coordinate needs at least two finite centers'
                )
            step = (values[-1] - values[0]) / (len(values) - 1)
            if step * direction <= 0:
                raise ValueError(
                    'Coordinates must be ascending in x and descending in y'
                )
    ...
    for source, start, last in zip(prepared, starts, ends):
        ...
        if not np.allclose(offset, rounded, rtol=0, atol=_TOLERANCE) or not np.allclose(
            (np.array(last) - origin) / spacing,
    ...
    attrs = {
        key: value
        for key, value in prepared[0].attrs.items()
        if key not in excluded
        and all(
            key in s.attrs and np.array_equal(value, s.attrs[key]) for s in prepared[1:]
        )
    }
    attrs['crs'] = crs
    name = (
        prepared[0].name if all(s.name == prepared[0].name for s in prepared) else None
    )
    return xr.DataArray(
        xr.Variable((y, x), indexing.LazilyIndexedArray(backend), attrs=attrs),
        coords={
            dim: origin[axis] + np.arange(shape[axis]) * spacing[axis]
