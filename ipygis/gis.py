...
class GIS:
    ...
    async def _request(
        self, operation, *, buffers=None, message_type='icechunk_request', **arguments
    ):
    ...
    async def decode_lzw(self, data: bytes, decoded_size: int) -> bytes:
        ...
        _, buffers = await self._request(
            'decode_lzw', decoded_size=decoded_size, buffers=[memoryview(data)]
        )
